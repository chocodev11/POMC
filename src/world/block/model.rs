...
pub fn load_all_block_textures(
    ...
    for_each_blockstate(assets_dir, asset_index, packs, |block_name, blockstate| {
...
pub fn bake_all_models(
    ...
    for_each_blockstate(assets_dir, asset_index, packs, |block_name, _| {
...
fn for_each_blockstate(
    ...
    let Some(blockstates_dir) = resolve_blockstates_dir(assets_dir, asset_index, packs) else {
...
fn resolve_blockstates_dir(
    ...
    if asset_index.is_some() {
        let test_path = resolve_asset_path(
            assets_dir,
            asset_index,
...
fn resolve_model(
    ...
    for _ in 0..20 {
        let Some(model) = load_model(&current_id, assets_dir, asset_index, cache, packs) else {
            break;
        };
...
fn load_model<'a>(
    ...
    let file_path = resolve_model_path(assets_dir, asset_index, &asset_key, packs)?;
...
fn resolve_model_path(
    ...
    let primary = resolve_asset_path(assets_dir, asset_index, asset_key, packs);
    if primary.exists() {
        return Some(primary);
    }
